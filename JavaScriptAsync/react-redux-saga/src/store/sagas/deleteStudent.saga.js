...
import { DELETE_STUDENT_BEGIN, deleteStudentSuccessAction, apiFailureAction } from "../reducers/studentReducer";
...
function* deleteStudentWorkerSaga(action) {
    try {
        ...
        yield put(apiFailureAction(`Error deleting student id=${action.id}.  ${error.message}.`));
    }
}
