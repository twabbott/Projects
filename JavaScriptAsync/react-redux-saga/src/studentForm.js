...
import { addStudentBeginAction, updateStudentBeginAction, hideFormAction, resetErrorMessageAction } from "./store";


const mapStateToProps = state => {
    return { 
        isEditMode: !!state.appState.studentToEdit,
        student: state.appState.studentToEdit, 
...
const mapDispatchToProps = dispatch => {
    return {
        onCreateBtnClicked: (firstName, lastName, male, uvuId, race, age, isVeteran) => {
            dispatch(addStudentBeginAction({firstName, lastName, male, uvuId, race, age, isVeteran}));
        },
        onSaveBtnClicked: (id, firstName, lastName, male, uvuId, race, age, isVeteran) => {
            dispatch(updateStudentBeginAction({id, firstName, lastName, male, uvuId, race, age, isVeteran}));
        },
        onCancelBtnClicked: () => {
...
class StudentForm extends Component {
    constructor(props) {
        ...
        this.state = {
            firstName: this.props.isEditMode? this.props.student.firstName: "",
            firstNameError: "",
            lastName: this.props.isEditMode? this.props.student.lastName:  "",
            lastNameError: "",
            gender: this.props.isEditMode ?
                (this.props.student.male ? "male" : "female") :
                "",
            genderError: "",
            uvuId: this.props.isEditMode? this.props.student.uvuId: "",
            uvuIdError: "",
            race: this.props.isEditMode? this.props.student.race: "",
            raceError: "",
            age: this.props.isEditMode? this.props.student.age: "",
            ageError: "",
            isVeteran: this.props.isEditMode? this.props.student.isVeteran : false
        };
        ...
        return (<div>
            ...
            <form>
                ...
                <button type="button" id="saveBtn" className="btn btn-primary"
                    onClick={this._onFormSubmit} disabled={this.props.updating}>Save</button>
                <button type="button" id="cancelBtn" className="btn btn-warning"
                    onClick={this.props.onCancelBtnClicked} disabled={this.props.updating}>Cancel</button>
                { this.props.updating && <span>Updating...</span> }
                { this.props.error && <div className="text-white bg-danger">{this.props.error}</div> }
            </form>
        </div>);
    }
}
